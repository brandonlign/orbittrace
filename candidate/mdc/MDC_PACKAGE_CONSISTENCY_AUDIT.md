# Package check

The draft submission files were checked against the recovered analysis records and the 95-row lookup table. The automated check completed 151 tests without a failure.

The checks covered:

- the 95 lookup rows and their arithmetic means;
- consistency between q, e, and a in the draft mean record;
- agreement between the JSON and legacy-text records;
- official MDC checker inputs and outputs;
- current official-catalogue provenance;
- exact primary event reproduction;
- the blind January–July rediscovery;
- the source-preserving, bootstrap, geographic, and sensitivity results;
- the CAMS and SonotaCo member sets; and
- the recovered source manifest.

The underlying machine-generated report was produced before the repository was reorganized, so some stored paths refer to the earlier canonical layout. The hashes and numerical comparisons refer to that frozen evidence package, not to the later wording and folder-name cleanup.

Passing this check means the draft files agree with one another and with the preserved computations. It does not provide independent scientific validation or permission to submit.
