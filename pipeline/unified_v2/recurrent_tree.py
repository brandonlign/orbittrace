...
def recurrent_stability(
    ...
    *,
...
) -> tuple[dict[float, float], dict[int, tuple[float, ...]], dict[int, tuple[int, ...]], dict[str, Any]]:
    """Compute a K-year, exposure-normalized lower-tail stability score.

    Each condensed-tree node receives one stability contribution per year,
    divided by that year's number of input events (or a supplied exposure).
    The score is the lower-tail quantile across years, with a support gate.
    This generalizes the v1 two-year minimum without allowing a very populous
    year to dominate the ranking.
    """
    ...
    totals = np.asarray(
        [float((years_array == year).sum()) for year in year_values],
        dtype=float,
    )
    if exposure is not None:
        totals = np.asarray([float(exposure[year]) for year in year_values], dtype=float)
    if np.any(totals <= 0):
        raise ValueError("every observing year must have positive exposure")
    if not 0.0 <= recurrence_quantile <= 1.0:
    ...
    annual_normalized = {
        parent: tuple((annual_mass[parent] / totals).tolist()) for parent in parents
    }
    counts = {
        parent: tuple(int(value) for value in descendant_counts[parent].tolist())
        for parent in parents
    }
    scores: dict[float, float] = {}
    support: dict[int, float] = {}
    for parent in parents:
        annual_counts = np.asarray(counts[parent], dtype=int)
        values = np.asarray(annual_normalized[parent], dtype=float)
        supported = annual_counts >= int(min_year_events)
        fraction = float(np.mean(supported))
        support[parent] = fraction
        if fraction < float(min_year_support_fraction):
            scores[float(parent)] = 0.0
            continue
        try:
            tail = float(np.quantile(values, recurrence_quantile, method="lower"))
        except TypeError:  # numpy < 1.22 compatibility
            tail = float(np.quantile(values, recurrence_quantile, interpolation="lower"))
        scores[float(parent)] = max(0.0, tail)
    diagnostics = {
        "year_values": list(year_values),
        "exposure": {str(year): float(total) for year, total in zip(year_values, totals)},
        "recurrence_quantile": float(recurrence_quantile),
        "min_year_support_fraction": float(min_year_support_fraction),
        "min_year_events": int(min_year_events),
        "node_support_fraction": {str(node): float(value) for node, value in support.items()},
    }
...
def _selected_eom_nodes(tree: np.ndarray, stability: dict[float, float]) -> tuple[int, ...]:
    """Mirror HDBSCAN's zero-epsilon EOM selection for provenance."""
...
def _candidate(
    ...
    *,
    ...
    recurrent_score: float | None = None,
    ordinary_score: float | None = None,
    annual_normalized: tuple[float, ...] | None = None,
    annual_counts: tuple[int, ...] | None = None,
    membership_probability: float | None = None,
) -> dict[str, Any]:
    ids = tuple(sorted(str(event_ids[index]) for index in members))
    counts = (
        tuple(int(np.sum(years[members] == year)) for year in year_values)
        if annual_counts is None
        else tuple(int(value) for value in annual_counts)
    )
    return {
        ...
        "members_by_year": {str(year): int(count) for year, count in zip(year_values, counts)},
        "annual_counts": list(counts),
        "annual_normalized_stability": None
        if annual_normalized is None
        else [float(value) for value in annual_normalized],
        "recurrent_stability": None if recurrent_score is None else float(recurrent_score),
        "ordinary_stability": None if ordinary_score is None else float(ordinary_score),
        "mean_membership_probability": None
        if membership_probability is None
        else float(membership_probability),
    }


def fit_recurrent_hierarchy(
    matrix: np.ndarray,
    years: np.ndarray,
    event_ids: np.ndarray,
    ...
    *,
    ...
    """Fit one hierarchy and return recurrent parents plus compact leaves."""
    ...
    if values.ndim != 2 or values.shape[0] != len(year_values) or values.shape[0] != len(ids):
        raise ValueError("matrix, years, and event_ids must have the same row count")
    if values.shape[0] < max(config.min_cluster_size * 2, config.min_samples + 1):
        raise ValueError("not enough rows for a recurrent hierarchy")
    if len(np.unique(year_values)) < 2:
        raise ValueError("at least two observing years are required")
    ...
    ordinary_stability = compute_stability(tree)
    ordinary_labels = _eom_labels(tree, ordinary_stability)
    if _canonical_partition(model.labels_) != _canonical_partition(ordinary_labels):
        raise RuntimeError("ordinary HDBSCAN-EOM path diverged from fitted assignments")
    recurrent, annual_normalized, annual_counts, recurrence_diag = recurrent_stability(
        tree,
        year_values,
        recurrence_quantile=config.recurrence_quantile,
        min_year_support_fraction=config.min_year_support_fraction,
        min_year_events=config.min_year_events,
    )
    recurrent_labels = _eom_labels(tree, recurrent)
    selected_nodes = _selected_eom_nodes(tree, recurrent)
    ...
    if positive != list(range(len(selected_nodes))):
        raise RuntimeError("recurrent EOM labels no longer align with selected nodes")
    ...
    for label, node in enumerate(selected_nodes):
        ...
        parents.append(
            _candidate(
                ...
                ids,
                year_values,
                tuple(sorted(int(value) for value in np.unique(year_values))),
                node_id=node,
                recurrent_score=recurrent[float(node)],
                ordinary_score=ordinary_stability[float(node)],
                annual_normalized=annual_normalized[node],
                annual_counts=annual_counts[node],
            )
        )

    leaves: list[dict[str, Any]] = []
    if include_leaves:
        leaf_labels, probabilities = _leaf_labels(tree, ordinary_stability)
        year_tuple = tuple(sorted(int(value) for value in np.unique(year_values)))
        exposure = {year: int(np.sum(year_values == year)) for year in year_tuple}
        for label in sorted(int(value) for value in np.unique(leaf_labels) if int(value) >= 0):
            members = np.flatnonzero(leaf_labels == label)
            if members.size < config.min_cluster_size:
                continue
            counts = tuple(int(np.sum(year_values[members] == year)) for year in year_tuple)
            normalized = tuple(count / exposure[year] for count, year in zip(counts, year_tuple))
            leaves.append(
                _candidate(
                    "LEAF2",
                    "leaf",
                    ...
                    year_values,
                    year_tuple,
                    annual_normalized=normalized,
                    annual_counts=counts,
                    membership_probability=float(np.mean(probabilities[members])),
                )
            )

    parents.sort(
        key=lambda item: (
            -float(item["recurrent_stability"] or 0.0),
            -float(item["ordinary_stability"] or 0.0),
    ...
    leaves.sort(
        key=lambda item: (
            -float(np.quantile(item["annual_normalized_stability"], config.recurrence_quantile)),
            -float(item["mean_membership_probability"] or 0.0),
            -int(item["member_count"]),
            item["family_id"],
        )
    )
    for rank, item in enumerate(parents, start=1):
        item["rank"] = rank
    for rank, item in enumerate(leaves, start=1):
        item["rank"] = rank
    diagnostics = {
        "events": int(values.shape[0]),
        "dimensions": int(values.shape[1]),
        "years": sorted(int(value) for value in np.unique(year_values)),
        "min_cluster_size": int(config.min_cluster_size),
        "min_samples": int(config.min_samples),
        "ordinary_candidates": int(len(set(int(value) for value in ordinary_labels if int(value) >= 0))),
        "recurrent_candidates": int(len(parents)),
        "leaf_candidates": int(len(leaves)),
        "mechanism_active": _canonical_partition(ordinary_labels)
        != _canonical_partition(recurrent_labels),
        "recurrence": recurrence_diag,
    }
