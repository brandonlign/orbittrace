...
def run(
    ...
    *,
    ...
    pretruth: dict[str, Any] = {
        "method": "v2 recurrent parent branch plus cross-fitted halo" if halo else "v2 recurrent parent branch",
    ...
    for route in ROUTES:
        ...
        if halo:
            candidates = [expand_candidate(candidate, matrix, years, event_ids, config) for candidate in parents]
        else:
            candidates = parents
...
def main() -> int:
    ...
    result = run(args.rows_root, args.truth_root, halo=args.halo)
