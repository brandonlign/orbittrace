...
def circular_difference_deg(left: np.ndarray, right: np.ndarray) -> np.ndarray:
    """Return ``left-right`` wrapped to ``[-180, 180)`` degrees."""
...
def periodic_physical6_from_raw(
    raw: np.ndarray,
    feature_scales: Sequence[float] = (3.5, 3.0, 2.5, 2.5),
) -> np.ndarray:
    """Transform ``(relative_lon, latitude, speed, solar_offset)`` rows.
    ...
    the successful physical scale used by the v1 exploratory lane.
    """
...
def periodic_physical6_from_mapping(
    rows: Sequence[Mapping[str, float]],
    feature_scales: Sequence[float] = (3.5, 3.0, 2.5, 2.5),
) -> np.ndarray:
    """Build the v2 representation from fair-benchmark row dictionaries."""
...
__all__ = [
    ...
    "periodic_physical6_from_mapping",
