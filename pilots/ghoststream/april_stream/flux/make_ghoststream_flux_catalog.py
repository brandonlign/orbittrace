...
# Conservative weak-shower binning. These are operational grouping controls,
# not fitted physical activity parameters.
BINNING = {
    "all_years": {
        "min_tap": 30,
        "min_meteors": 20,
...
def make_entry(mass_index: float) -> str:
    ...
    fields = [
        ...
        "annual",
        f"{SOL_REFERENCE:.3f}",
        "1.0",  # placeholder forecast ZHR; not used for the physical result
        "0.0",  # placeholder rise coefficient; use explicit activity bounds
        "0.0",  # placeholder fall coefficient; use explicit activity bounds
        f"{r:.8f}",
        "0.0",  # invoke the RMS velocity-dependent height model
        f"{ASSOCIATION_RADIUS:.2f}",
        repr(BINNING),
    ]
    return "|".join(fields)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--rms-root", type=Path, required=True,
                        help="Root of a current CroatianMeteorNetwork/RMS checkout")
    parser.add_argument("--mass-index", type=float, required=True,
                        help="Mass index s. No default is provided intentionally.")
...
def main() -> int:
    ...
    metadata = {
        ...
        "mass_index": args.mass_index,
        "population_index": population_index(args.mass_index),
        "internal_code": INTERNAL_CODE,
        "official_iau_status": "unassigned_candidate",
        "activity_bounds_solar_longitude_deg": [SOL_BEGIN, SOL_END],
        "reference_solar_longitude_deg": SOL_REFERENCE,
        "radiant_at_reference": {
            "ra_deg": RA_REFERENCE,
            "dec_deg": DEC_REFERENCE,
            "vg_km_s": VG_REFERENCE,
        ...
        "warning": (
            "ZHR and double-exponential activity coefficients in the catalogue entry are operational "
            "placeholders. Use explicit activity bounds; infer physical flux/ZHR from the returned TAP "
            "and calibrated meteor counts."
        ),
    }
    (args.output_dir / "ghoststream_flux_catalog_metadata.json").write_text(
