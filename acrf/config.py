...
class ACRFConfig:
    """One target-free configuration used on every survey and benchmark."""
    ...
    def __post_init__(self) -> None:
        ...
        if not 0.0 < self.hierarchy_window_stride_deg <= self.hierarchy_window_width_deg:
            raise ValueError("hierarchy_window_stride_deg must lie in (0, width]")
        ...
        if self.hierarchy_max_candidate_members < self.min_cluster_size:
            raise ValueError("hierarchy_max_candidate_members is too small")
        if self.global_anchor_count < 1:
            raise ValueError("global_anchor_count must be positive")
        ...
        if self.calibration_permutations < 99:
            raise ValueError("calibration_permutations must be at least 99")
        if self.calibration_correction != "benjamini_yekutieli":
            raise ValueError("ACRF freezes the dependence-safe BY correction")
        if not 0.0 < self.halo_core_tail_alpha < 1.0:
            raise ValueError("halo_core_tail_alpha must lie in (0, 1)")
        if self.halo_cutoff_inflation < 1.0:
            raise ValueError("halo_cutoff_inflation must be at least one")
        if self.halo_iterations < 1:
            raise ValueError("halo_iterations must be positive")
        if self.halo_orbit_distance_max <= 0.0 or self.halo_scale_floor <= 0.0:
            raise ValueError("halo distance and scale constants must be positive")
        ...
        if self.halo_background_shifts < 3:
            raise ValueError("halo_background_shifts must be at least three")
