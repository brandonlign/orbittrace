...
from pipeline.unified_v2.crossfit_membership import (
    _bh_qvalues,
    _distances,
    _membership_pvalues,
...
def _permutation_background_pvalues(
    ...
    if held_values.shape[1] != 6:
        raise ValueError("v3.3 permutation membership requires periodic_physical6")
    order = np.asarray(
        sorted(
            range(len(event_ids)),
            key=lambda index: hashlib.sha256(str(event_ids[index]).encode()).digest(),
        ),
        dtype=int,
    )
    solar_pair = held_values[order, :2].copy()
    null_scores = []
    for step in range(1, int(shifts) + 1):
        offset = max(1, int(round(step * len(order) / (int(shifts) + 1))))
        permuted = held_values.copy()
        permuted[order, :2] = np.roll(solar_pair, offset, axis=0)
        null_scores.append(_distances(permuted, center, scale, None))
    pooled = np.sort(np.concatenate(null_scores))
    lower_equal = np.searchsorted(pooled, scores, side="right")
    return (1.0 + lower_equal.astype(float)) / (len(pooled) + 1.0)
...
def expand_candidate(
    ...
    for _iteration in range(int(config.halo_iterations)):
        ...
        for year in sorted(int(value) for value in np.unique(year_array)):
            heldout = np.flatnonzero(year_array == year)
            training = snapshot[year_array[snapshot] != year]
            if len(training) < config.halo_min_training_members or not len(heldout):
                folds[str(year)] = {
                    "skipped": True,
                    "training_active_members": int(len(training)),
            ...
            cutoff = float(
                np.quantile(core_scores, 1.0 - config.halo_core_tail_alpha)
                * config.halo_cutoff_inflation
            )
            scores = _distances(values[heldout], center, scale, None)
            conformity = _membership_pvalues(core_scores, scores)
            background_p = _permutation_background_pvalues(
                values[heldout],
                scores,
                center,
                scale,
            ...
            folds[str(year)] = {
                ...
                "core_cutoff": cutoff,
                "mean_accepted_conformity": float(np.mean(conformity[accepted_mask]))
                if bool(accepted_mask.any())
                else None,
                "max_accepted_background_q": float(np.max(background_q[accepted_mask]))
                if bool(accepted_mask.any())
                else None,
            }
    ...
    result["crossfit_halo"] = {
        ...
        "core_tail_alpha": float(config.halo_core_tail_alpha),
        "cutoff_inflation": float(config.halo_cutoff_inflation),
        "iterations_run": int(len(iteration_folds)),
        "iterations": iteration_folds,
