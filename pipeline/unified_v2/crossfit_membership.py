...
def _membership_pvalues(core_scores: np.ndarray, scores: np.ndarray) -> np.ndarray:
    """Conformal similarity p-values: large means compatible with the core."""
...
def expand_candidate(
    ...
    *,
    ...
    retain_member_diagnostics: bool = True,
) -> dict[str, Any]:
    """Expand a hierarchy core with held-out-year robust conformal membership.

    For every held-out year, the robust centre and diagonal scale are fitted
    only on candidate core members from the other years.  Event uncertainties
    ...
    for iteration in range(int(config.halo_iterations)):
        ...
        for year in sorted(int(value) for value in np.unique(year_values)):
            ...
            if len(training) < config.halo_min_training_members or heldout.size == 0:
                folds[str(year)] = {
                    "training_core": int(len(training)),
                    "heldout_events": int(len(heldout)),
                    "skipped": True,
                    "reason": "insufficient_crossfit_training",
                }
                continue
            ...
            fold: dict[str, Any] = {
                ...
                "new_events": int(len(fold_additions)),
                "skipped": False,
                "center": center.tolist(),
                "scale": scale.tolist(),
                "core_cutoff": cutoff,
                "accepted_fraction": float(len(accepted) / len(heldout)),
    ...
    result["crossfit_halo"] = {
        ...
        "scale_floor": float(config.halo_scale_floor),
        "density_fdr_enforced": bool(config.halo_enforce_density_fdr),
        "density_fdr": float(config.halo_density_fdr),
        "method": "leave-one-year-out robust diagonal conformal envelope",
    }
    return result
